...
// ---------------------------------------------------------------------------
// ANSI Shadow Font — identical to the plugin's serve-card.mjs
// ---------------------------------------------------------------------------
const FONT = {
  'A': [' █████╗ ','██╔══██╗','███████║','██╔══██║','██║  ██║','╚═╝  ╚═╝'],
  'B': ['██████╗ ','██╔══██╗','██████╔╝','██╔══██╗','██████╔╝','╚═════╝ '],
...
function renderAsciiLine(text) {
  ...
  return lines.map(parts => parts.join('')).filter(l => l.trim()).join('\n');
}

function renderAsciiName(name) {
  // Split into two lines if the name exceeds ~10 characters
  if (name.length > 10 && name.includes(' ')) {
    const spaceIdx = name.indexOf(' ');
    const first = name.slice(0, spaceIdx);
    const rest = name.slice(spaceIdx + 1);
    return renderAsciiLine(first) + '\n' + renderAsciiLine(rest);
  }
...
export default function CardPage() {
  ...
  const asciiName = renderAsciiName(dev.name);
      ...
      <div style={{ display: "flex", flexWrap: "nowrap", gap: 32, alignItems: "flex-start" }} className="card-layout">
        ...
        <div style={{ flex: "1 1 0", minWidth: 0, maxWidth: hasSidebar ? 720 : 720 }}>
          <div style={{
            ...
            {/* Titlebar */}
            <div style={{ background: "#161b22", padding: "12px 16px", display: "flex", alignItems: "center", gap: 8, borderBottom: "1px solid #30363d", userSelect: "none" }}>
              <div style={{ width: 12, height: 12, borderRadius: "50%", background: "#ff5f56" }} />
              <div style={{ width: 12, height: 12, borderRadius: "50%", background: "#ffbd2e" }} />
              <div style={{ width: 12, height: 12, borderRadius: "50%", background: "#27c93f" }} />
              <span style={{ flex: 1, textAlign: "center", color: "#484f58", fontFamily: mono, fontSize: 13, letterSpacing: 0.5 }}>devcard</span>
              <div style={{ width: 52 }} />
            </div>
            ...
            <div className="card-body-content" style={{ padding: "28px 32px 24px" }}>
              {/* ASCII Art Name */}
              <pre className="ascii-card-name" style={{
                color: CLAUDE_ORANGE,
                fontFamily: mono,
                fontSize: 9,
                lineHeight: 1.15,
              ...
              {/* Archetype — Claude calls you */}
              {dev.archetype && (
                <div style={{ fontFamily: mono, fontSize: 13, marginBottom: 16 }}>
                  <span style={{ color: "#6e7681" }}>Claude calls you</span>{" "}
